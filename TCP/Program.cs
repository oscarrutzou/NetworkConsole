...
namespace TCP
{
    ...
    internal static class Program
    {
        ...
        static TcpListener server = new TcpListener(IPAddress.Any, 12000);

        static TCPAnswerKeyMsg _answerKeyMsg;
                ...
                while (client.Connected)
                {
                    ...
                    switch (recievedType)
                    {
                        ...
                        case TCPMessagesTypes.ChatMessage:
                            TCPChatMsg chatMsg = MessagePackSerializer.Deserialize<TCPChatMsg>(messageBytes);
                            string sentMes = $"{myInfo.Name}: {chatMsg.Message}";
                        ...
                        case TCPMessagesTypes.C_RequestListMsg:
                            ...
                            //    HelloMessage hMes = MessagePackSerializer.Deserialize<HelloMessage>(messageBytes);
                            //    Console.WriteLine("New user joined!! Welcome: " + hMes.UserName);
                            //    myInfo.Name = hMes.UserName;
                            //    connectedClients.SendMessageToAll($"{hMes.UserName} joined the server! Say hello");
                            //    break;
                            //case MessageType.GlobalMessage:
                            //    //send to all connected
                            //    GlobalMessage gMes = MessagePackSerializer.Deserialize<GlobalMessage>(messageBytes);
                            //    HandleGlobelMessage(gMes, clientGuid);
                            //    break;
                            //case MessageType.DirectMessage:
                            //    DirectMessage dMes = MessagePackSerializer.Deserialize<DirectMessage>(messageBytes);
                            //    HandleDirectMessage(dMes, clientGuid);
                            //    break;
                            //case MessageType.List:
                            //    //returns message to sender.
                            //    myInfo.SendMessage("Online Users:\n" + connectedClients.GetNameOfAllAsSingleString());
                            //    break;
                    }
                }
            }
